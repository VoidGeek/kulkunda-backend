...
const { parseCookies, setCookie, destroyCookie } = require('next-cookies'); // Import next-cookies
...
exports.register = asyncErrHandler(async (req, res, next) => {
  ...
  if (password !== confirmpassword) {
    return next(errorHandler(400, 'The password and confirmpassword should be same'));
  }
  const afterAtSymbol = email.split('@')[1];
  let role;
  if (afterAtSymbol === 'kulkundabasaweshwara.com') {
    role = 'admin';
  } else {
    role = 'user';
  }
  ...
  if (!user) {
    return next(errorHandler(400, "User isn't created"));
  }
...
exports.updatePassword = asyncErrHandler(async (req, res, next) => {
  ...
  if (!isverfied) {
    return next(errorHandler(403, 'Wrong password try again'));
  }
...
exports.numberOfUsers = asyncErrHandler(async (req, res, next) => {
  ...
  if (!length) {
    return next(errorHandler(403, 'There are no users in the database'));
  }
...
exports.checkcookies = asyncErrHandler(async (req, res, next) => {
  ...
  if (!yourCookie) {
    return next(errorHandler(400, 'No cookie'));
  }
